#include <iostream>                     //Nimarpreet Singh
#include <queue>                        //SID:21105075 ECE
using namespace std;

//Creating class of Tree Node
class Node{
    public:
        int data;
        Node*left;
...
//Function for inserting data in a Binary Search Tree
...
int main()
{
    //Taking number of element in array as input from user
    int n;
    ...
    cout<<endl;
    Node*root=NULL;
    //Inserting an element of the array in Binary Search Tree
    for(int i=0;i<n;i++){
        root=insert_in_BST(root,arr[i]);
    }
    //Displaying the Binary Search Tree using Level Order Traversal
    ...
    //deleting node with value key from Binary Search Tree
    root=delete_node(root,key);
    //Displaying Binary Search Tree after deletion of node
...
search tree than the tree has space complexity of O(n) and the arrayhas also the space complexity of O(n)
*/
