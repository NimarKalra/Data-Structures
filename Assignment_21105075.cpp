...
using namespace std;             //Branch: ECE 
                                 //SID: 21105075
/*
QUESTION:
...
void bubble_sort(int arr[],int n){
    ...
    while(t>0){
        while(j<=t){
            if(arr[i]>arr[j]){swap(arr[i],arr[j]);swaps++;}
            i++,j++;
            comp++;
        }
        t--;
...
int minimum_index(int arr[],int si,int ei){
    //Initialising minimum value as first element of array
    int m=arr[si];             //m=minimum value in array
    int minimum_index=si;           //minimum_index=index holding minimum value
    for(int i=si;i<=ei;i++){
        int prev_min=m;
        m=min(m,arr[i]);
        if(m<prev_min){minimum_index=i;}
    }
...
void selection_sort(int arr[],int n){
    ...
    int comp=0;
    while(si<ei){
    int mi=minimum_index(arr,si,ei);  //mi=index holding minimum value
    comp=comp+(ei-si+1);
...
int main(){
    ...
    int n;
    //taking input number of elements as input from the user
...
TIME COMPLEXITY OF BUBBLE SORT
   ...
   times and second while loop will run n-2 times and so on till 1 time at last iteration.
